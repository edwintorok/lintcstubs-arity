...
Arity > 5 is implemented differently in bytecode and native code:
  ...
  CAMLprim value add_nat_bytecode(value *, int);

Native code can take some arguments unboxed, but that would require a typedtree
to be done correctly (it is possible to redefine 'type int = string'), so print just the bytecode variant here.
  $ cat >test.ml <<EOF
  > external foo
  > :  (float [@unboxed])
  > -> (float [@unboxed])
  > -> (float [@unboxed])
  > = "foo_byte" "foo"
  > external foo2 : float -> float -> float = "foo2_byte" "foo2" [@@unboxed]
...
Exitcode is set on parse error
  $ rm -f test.cmt
  $ touch test.cmt
  $ lintcstubs_arity_cmt test.cmt
  /* AUTOGENERATED FILE, DO NOT EDIT */
